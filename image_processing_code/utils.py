...
def draw_axis(img, yaw, pitch, roll, ptx=None, pty=None, size=100):
    ...
    x1 = size * (cos(yaw) * cos(roll)) + ptx
    y1 = size * (cos(pitch) * sin(roll) + cos(roll) * sin(pitch) * sin(yaw)) + pty

    # Y-Axis | drawn in green
    #        v
    x2 = size * (-cos(yaw) * sin(roll)) + ptx
    y2 = size * (cos(pitch) * cos(roll) - sin(pitch) * sin(yaw) * sin(roll)) + pty
...
# def create_mask(img, yaw, pitch, roll, tdx=None, tdy=None, size=100):
#
#     pitch = pitch * np.pi / 180
#     yaw = -(yaw * np.pi / 180)
#     roll = roll * np.pi / 180
...
#         pty = height / 2
#
#     cntr_pnt = (int(ptx), int(pty))
#     pt1 =
#     pt2 = (int(x3), int(y3))
#
#     mask = np.full(img.shape,0, dtype=np.uint8)
#     roi = np.array([[, cntr_pnt,]], dtype=np.int32)
